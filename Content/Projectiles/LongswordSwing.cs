...
namespace aftermath.Content.Projectiles
{
    public class LongswordSwing : ModProjectile
    {
        ...
        public override string Texture => $"Terraria/Images/Item_{ItemID.Katana}"; // uses whatever texture the longsword item uses (for now, katana)
        ...
        {
            Owner.itemAnimation = 10; // changing these will extend the length of time it takes to swing again
            Owner.itemTime = 10;      // always keep them the same
        ...
        private void ExecuteSwing()
        {
            ...
            {
                //Progress = MathHelper.SmoothStep(0, 0, (1f - UNWIND) * Timer / execTime);
                //Vector2 playerCenter = player.RotatedRelativePoint(player.MountedCenter, reverseRotation: false, addGfxOffY: false);
                //Projectile.Center = playerCenter + Projectile.velocity * (Timer);
            }
